...
Branch feature work from `dev` and open a pull request into `dev`. Promotion to `main` is a reviewed pull request from `dev`; keep both long-lived branches. Do not develop directly on `main`, force-push a release tag, or edit an uploaded release asset.
...
Use descriptive commits, for example `feat: clarify narration recovery` or `fix: preserve plugin references in release archives`. Dependabot targets `dev`. Actions are pinned to immutable commits and updated through reviewed pull requests.
