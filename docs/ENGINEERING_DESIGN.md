...
Configuration changes are transactions: stage and validate, record a private journal, check the current file again, replace atomically, install the matching skills, verify, then write a receipt. Multi-file changes are not filesystem-atomic. Recover interrupted work from the journal. Delete completed journals; retain only unresolved recovery state with owner-only permissions because host configuration can contain credentials. Stage on the destination filesystem so atomic replacement also works with profiles on another volume. A pre-write hash check is not a compare-and-swap primitive; defer if the host is concurrently writing and a shared lock is unavailable.
