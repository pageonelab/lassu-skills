...
The new scenario rows were reviewed against the skill instructions for fresh installation, upgrading an older skills-only plugin, an existing manual connection, pending/revoked authorization and service 404s. This was a source/contract walkthrough, not an independent agent execution or an interactive OAuth test. The application-managed personal skill payload remains pinned separately in the product repository and should be updated from an immutable public commit after this change is published.

## Local App Draw preference (0.3.0)

The package's 15 tests pass, including local-before-remote metadata, deferred authentication, reproducible archives and preserved remote endpoint configuration. The official Codex plugin validator and skill validator pass. A fresh temporary Codex CLI 0.153.4 profile installed this local marketplace and reported version 0.3.0 with `authPolicy: ON_USE`; it discovered the remote endpoint while remaining unauthenticated, without opening a consent flow. This establishes plugin loader metadata, not an authenticated remote drawing session.

The matching product change supplies the native Draw bridge. Its API tests use desktop sessions without OAuth registration, exercise private board creation, retries, updates, undo, render delivery and access denial, and retain the remote OAuth regressions. Local MCP SDK tests discover 17 Draw tools and verify IPC calls, error preservation and inline preview delivery. App/API rollout and fresh host tool loading remain necessary before an older installed App can use local Draw.

## Plugin and App skill handoff (0.3.1)

On September 24, 2026, fresh temporary profiles installed `lassu@lassu` 0.3.0 from the GitHub marketplace with Codex CLI 0.155.0-alpha.16 (bundled with the desktop app) and Claude Code 2.1.119, then upgraded to the 0.3.1 working tree through a local marketplace with each host's update command. Claude Code's validator passed for the marketplace and plugin. Both hosts registered the remote `lassu-draw` server as unauthenticated. Production discovery returned the expected 401 challenge, protected-resource metadata and authorization-server metadata with dynamic registration and S256 PKCE. Account consent was not performed.

With the installed Lassu 0.4.0 App, connecting after the plugin created no personal copies. Connecting before installing the plugin left both copies loaded: Claude Code listed `lassu-draw` and `lassu:lassu-draw`, and Codex listed six Lassu skills. The App's refresh reported needs-attention and reconnecting failed. The matching product change hands unchanged App-owned copies to the plugin; with it, one refresh removed them and each host listed three plugin skills.

A Codex agent given only the 0.3.0 walkthrough reference chose the remote Draw server and requested OAuth in 5 of 5 runs, although a signed-in local App was available. With the 0.3.1 reference it chose the local App and did not request OAuth in 5 of 5 runs. Codex (installed plugin) and Claude Code (App-managed skills) sessions each called local `lassu_draw_get_context` successfully. The production Draw renderer reported no heartbeat, so preview and presentation rendering were unavailable; no board was created or rendered.

All 15 package tests, generated-file parity and the release check passed locally.
